  ...
  <div id="rooms_box">
    <div class="room_list">
      ...
      <ul>
        <c:forEach items="${houses }" var="i">
	  	<a href="roomDetail.do?house_no=${i.house_no }">
			<li>
            <figure>
              <img alt="썸네일" src="img/room_list_img.jpg">
            </figure>
            <div class="info">
    ...
    <div class="paging">
      ...
      <c:if test="${page gt 10 }">
        <button onclick="location.href='houses.do?page=${page-10 }'"><<</button>
      </c:if>
      <c:if test="${page gt 1 }">
        <button onclick="location.href='houses.do?page=${page-1 }'"><</button>
      </c:if>
      <c:forEach var="i" begin="${startPage }" end="${endPage }">
        <c:if test="${i eq page }">
          <button id="click"
            onclick="location.href='houses.do?page=${i }'">${i }</button>
        </c:if>
        <c:if test="${i ne page }">
          <button onclick="location.href='houses.do?page=${i }'">${i }</button>
        </c:if>
      </c:forEach>
      ...
      <c:if test="${page eq totalPage-10 }">
        <button onclick="location.href='houses.do?page=${totalPage }'">>></button>
      </c:if>
